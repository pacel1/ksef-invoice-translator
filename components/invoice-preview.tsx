...
function originalInvoice(invoice: Invoice): Invoice {
  return {
    ...invoice,
    items: invoice.items.map((item) => withoutKeys(item, ["translatedName", "translatedUnit"])),
    additionalDescriptions: invoice.additionalDescriptions?.map((entry) =>
      withoutKeys(entry, ["translatedKey", "translatedValue"])
    ),
    settlements: invoice.settlements
      ? {
          ...invoice.settlements,
          charges: invoice.settlements.charges?.map((line) => withoutKeys(line, ["translatedReason"])),
          deductions: invoice.settlements.deductions?.map((line) => withoutKeys(line, ["translatedReason"]))
        }
      : undefined,
    orders: invoice.orders?.map((order) => ({
      ...order,
      lines: order.lines?.map((line) => withoutKeys(line, ["translatedName", "translatedUnit"]))
    })),
