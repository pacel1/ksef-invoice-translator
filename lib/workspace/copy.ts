// Verbatim move of the bilingual copy dictionary previously inlined in app/page.tsx.
// Phase 2 lifts it here so the workspace component and the landing page can share strings.

export type UiLanguage = "pl" | "en";

export const copy = {
  pl: {
    navProduct: "Produkt",
    navHow: "Jak działa",
    navPricing: "Cennik",
    navFaq: "FAQ",
    uiLanguage: "Język interfejsu",
    badge: "Narzędzie dla polskich firm pracujących z KSeF",
    headline: "Tłumacz faktury KSeF na profesjonalne faktury dla zagranicznych kontrahentów.",
    subheadline:
      "Wgraj FA(3) XML faktury KSeF, sprawdź czytelny podgląd, przetłumacz opisy i wyeksportuj estetyczny PDF bez logowania do KSeF, ERP ani systemu księgowego.",
    ctaUpload: "Wgraj fakturę",
    ctaHow: "Zobacz proces",
    trust1: "Nie wystawia faktur",
    trust2: "Nie łączy się z KSeF",
    trust3: "Nie zmienia danych finansowych",
    uploadTitle: "Wgraj KSeF FA(3) XML",
    uploadHelp: "Przeciągnij plik tutaj albo wybierz z dysku.",
    parserFeature: "Parser FA(3) XML KSeF",
    translationFeature: "Tłumaczenia etykiet i opisów",
    pdfFeature: "Eksport PDF z polskimi znakami i QR",
    targetLanguage: "Język tłumaczenia faktury",
    bilingual: "Dwujęzycznie: wybrany język + polski",
    translate: "Tłumacz opisy",
    download: "Pobierz PDF",
    parsing: "Analizuję fakturę KSeF...",
    empty: "Wgraj fakturę FA(3) XML z KSeF, aby wygenerować podgląd w języku",
    noAi:
      "Etykiety są tłumaczone słownikowo. Dodaj OPENAI_API_KEY, aby tłumaczyć opisy pozycji, jednostki, notatki i stopkę.",
    parsePdfFailed: "Nie udało się odczytać pliku.",
    translationFailed: "Tłumaczenie nie powiodło się.",
    pdfFailed: "Generowanie PDF nie powiodło się.",
    workspaceTitle: "Konwerter faktury",
    workspaceLead:
      "To główne narzędzie aplikacji. Wgraj plik, wybierz język kontrahenta, zdecyduj czy dokument ma być jedno- czy dwujęzyczny i pobierz gotowy PDF.",
    howTitle: "Jak korzystać",
    howLead: "Proces jest prosty i nie wymaga wdrożenia ani integracji.",
    steps: [
      ["Wgraj plik", "Obsługiwany jest XML FA(3) pobrany z KSeF."],
      ["Sprawdź podgląd", "Aplikacja normalizuje dane faktury do jednego modelu i pokazuje je w czytelnym układzie."],
      ["Przetłumacz treść", "Stałe etykiety pochodzą ze słowników, a AI tłumaczy tylko opisy, notatki i wolny tekst."],
      ["Pobierz PDF", "Eksport zawiera dane stron, pozycje, płatności, stopkę, QR KSeF i informację o reprezentacji tłumaczonej."]
    ],
    valueTitle: "Zaprojektowane pod komunikację z kontrahentami",
    valueLead:
      "To nie jest księgowość ani ERP. To wyspecjalizowane narzędzie do przygotowania zrozumiałej reprezentacji faktury dla odbiorcy zagranicznego.",
    values: [
      ["Bezpieczny zakres", "Nie wystawiamy, nie księgujemy i nie modyfikujemy faktury źródłowej."],
      ["Kontrola wartości", "Kwoty, daty, numery faktur, NIP, IBAN i stawki VAT nie są tłumaczone przez AI."],
      ["Europejskie języki", "Obsługa najważniejszych języków europejskich z możliwością szybkiego dodania kolejnych."],
      ["KSeF-ready", "Mapper obsługuje FA(3), faktury zaliczkowe, płatności, rachunki bankowe, stopkę i QR weryfikacyjny."]
    ],
    pricingTitle: "Cennik MVP",
    pricingLead: "Prosty model dla firm, które chcą szybko wysyłać czytelne faktury do partnerów zagranicznych.",
    plans: [
      ["Starter", "0 zł", "Testy i walidacja procesu", ["Upload XML", "Podgląd faktury", "Eksport przykładowy", "Brak kont użytkowników"]],
      ["Business", "149 zł / mies.", "Dla zespołów sprzedaży i administracji", ["Nielimitowane tłumaczenia", "PDF z brandingiem", "Historia lokalna lub firmowa", "Priorytetowe języki UE"]],
      ["Enterprise", "Indywidualnie", "Dla organizacji z większym wolumenem", ["SLA i wdrożenie", "Polityki bezpieczeństwa", "Integracje wewnętrzne", "Dedykowane szablony PDF"]]
    ],
    faqTitle: "Najczęstsze pytania",
    faqs: [
      ["Czy aplikacja łączy się z KSeF?", "Nie. Użytkownik sam wgrywa XML. Aplikacja tylko parsuje, tłumaczy, renderuje i eksportuje."],
      ["Czy to zastępuje księgowość?", "Nie. To czytelna reprezentacja faktury dla kontrahenta, a nie system księgowy."],
      ["Co tłumaczy AI?", "Wyłącznie wolny tekst: opisy pozycji, usług, notatki, stopkę i instrukcje płatnicze."],
      ["Czy kwoty lub numery mogą zostać zmienione?", "Nie powinny. Dane finansowe, identyfikatory, daty, waluty, IBAN i stawki VAT są przepisywane bez tłumaczenia AI."]
    ],
    seoTitle: "Tłumacz faktur KSeF, viewer FA(3), XML invoice to PDF",
    seoBody:
      "KSeF Invoice Translator pomaga polskim firmom przygotować profesjonalne tłumaczenie faktury KSeF dla kontrahenta. Narzędzie obsługuje FA(3) XML invoice, Polish invoice translation, translate KSeF invoice oraz eksport invoice PDF dla partnerów międzynarodowych.",
    balanceFree: "Darmowy kredyt",
    balanceFreePaid: "kredytów",
    outOfCreditsTitle: "Brak kredytów",
    outOfCreditsBody:
      "Wykorzystałeś darmową fakturę w tym miesiącu i nie masz pakietu kredytów. Kup pakiet, aby kontynuować — pierwszy kredyt darmowy odnawia się 1. dnia każdego miesiąca.",
    buyCredits: "Kup pakiet",
    cancel: "Anuluj",
    close: "Zamknij",
    drawerTitle: "Doładuj kredyty",
    billingPlaceholderTitle: "Pakiety kredytów",
    billingPlaceholderBody: "Sklep z pakietami będzie dostępny w następnej fazie.",
    billingTitle: "Pakiety kredytów",
    billingSubtitle: "Pierwszy kredyt darmowy odnawia się 1. dnia każdego miesiąca. Kredyty z pakietów nie wygasają.",
    billingBandPaidLabel: "kredytów",
    billingBandFreeLabel: "darmowy w tym miesiącu",
    billingBandNextFreeLabel: "Następne darmowe odnowienie:",
    billingIncludedHeading: "Co dostajesz w cenie?",
    billingVatNote: "Wszystkie ceny netto. VAT 23% naliczany przy zakupie.",
    billingRefundPolicy: "Niewykorzystane kredyty nie wygasają. Zwroty na życzenie w 14 dni.",
    pickPackage: "Wybierz wielkość pakietu",
    unitPrice: "za fakturę (netto)",
    total: "Razem (netto)",
    totalWithTax: "Z 23% VAT",
    continueToCheckout: "Przejdź do płatności",
    purchaseHistory: "Historia zakupów",
    purchaseDate: "Data",
    purchaseSize: "Pakiet",
    purchaseTotal: "Kwota",
    purchaseStatus: "Status",
    purchaseStatusPaid: "Opłacony",
    purchaseStatusPending: "W toku",
    purchaseStatusFailed: "Nieudany",
    purchaseStatusRefunded: "Zwrócony",
    fakturaHeader: "Faktura",
    fakturaInProgress: "W toku",
    fakturaIssued: "Wystawiona ✓",
    fakturaError: "Błąd",
    fakturaDownload: "Pobierz PDF →",
    paymentSuccessTitle: "Płatność zakończona",
    paymentSuccessBody: "Kredyty zostały dodane do Twojego konta.",
    paymentCancelledTitle: "Płatność anulowana",
    paymentCancelledBody: "Płatność została anulowana. Możesz spróbować ponownie.",
    topUp: "Doładuj",
    creditsExhaustedShort: "Brak kredytów",
    lowBalanceBannerTitle: "Brak kredytów",
    lowBalanceBannerBody:
      "Wykorzystałeś darmową fakturę w tym miesiącu i nie masz pakietu kredytów. Kup pakiet, aby przesłać kolejną fakturę.",
    newInvoice: "Nowa faktura",
    recentHeading: "Ostatnie",
    allArchive: "Cały archiwum",
    helpLabel: "Pomoc",
    contactLabel: "Kontakt",
    tryWithSample: "Spróbuj z przykładową fakturą",
    onboardingTitle: "Co dostajesz",
    onboardingItem1: "1 darmowa faktura w miesiącu",
    onboardingItem2: "Tłumaczenie do 20+ języków",
    onboardingItem3: "Dwujęzyczny PDF z kodem QR KSeF",
    onboardingItem4: "Bez logowania do KSeF, bez integracji",
    cached: "zapisane",
    moreLanguages: "Więcej języków",
    originalPolish: "Polski",
    navWorkspace: "Workspace",
    navHistory: "Historia",
    signOut: "Wyloguj",
    accountTitle: "Konto",
    accountProfileHeading: "Profil",
    accountEmailLabel: "E-mail",
    accountEmailHelp: "E-mail logowania nie można zmienić.",
    accountLocaleLabel: "Język interfejsu",
    accountDisplayNameLabel: "Nazwa wyświetlana (opcjonalnie)",
    accountDisplayNameHelp: "Używana w transakcyjnych e-mailach.",
    accountFirstNameLabel: "Imię",
    accountLastNameLabel: "Nazwisko",
    accountFullNameHelp: "Imię i nazwisko trafiają na każdą przetłumaczoną fakturę jako osoba zatwierdzająca — wymóg Ministerstwa Finansów.",
    nameCaptureTitle: "Imię i nazwisko zatwierdzającego tłumaczenie",
    nameCaptureBody:
      "Dokumenty powinny zawierać informację o osobie, która dokonała/sprawdziła tłumaczenie — to jeden z warunków Ministerstwa Finansów dla uznania tłumaczonego dokumentu. Po wpisaniu danych zastąpimy adres e-mail na fakturze Twoim imieniem i nazwiskiem.",
    nameCaptureFirstNameLabel: "Imię",
    nameCaptureLastNameLabel: "Nazwisko",
    nameCaptureSave: "Zapisz i kontynuuj",
    nameCaptureSaving: "Zapisuję…",
    nameCaptureDismiss: "Może później",
    nameCaptureError: "Nie udało się zapisać. Spróbuj ponownie.",
    accountSaveButton: "Zapisz zmiany",
    accountSavingButton: "Zapisuję…",
    accountSaveSuccess: "Zapisano.",
    accountSaveError: "Nie udało się zapisać zmian.",
    accountExportHeading: "Eksport danych (RODO)",
    accountExportBody: "Pobierz pełny eksport swoich danych w formacie JSON.",
    accountExportButton: "Pobierz dane (JSON)",
    accountExportPreparing: "Przygotowuję…",
    accountDangerHeading: "Strefa niebezpieczna",
    accountDeleteAccountTitle: "Usuń konto",
    accountDeleteAccountBody: "Wszystkie twoje dane (faktury, tłumaczenia, historia zakupów) zostaną trwale usunięte.",
    accountDeleteAccountButton: "Usuń konto",
    accountDeleteConfirmTitle: "Potwierdź usunięcie konta",
    accountDeleteConfirmBody: "Wpisz swój adres e-mail, aby potwierdzić. Tej operacji nie można cofnąć.",
    accountDeleteConfirmPlaceholder: "Wpisz adres e-mail",
    accountDeleteConfirmAction: "Tak, usuń trwale moje konto",
    accountDeleteCancel: "Anuluj",
    // -------------------------------------------------------------------
    // Tłumacz wizard (spec 2026-05-20). Three-step flow:
    //   Step 1 Wybierz pliki  →  Step 2 Język i format  →  Step 3 Tłumaczenie
    // Keys live here (not in a separate file) so the existing copy[uiLang]
    // pattern keeps working — components read t.uploadHeading etc.
    // -------------------------------------------------------------------
    wizardStepUpload: "Wybierz pliki",
    wizardStepLanguage: "Język i format",
    wizardStepDelivery: "Tłumaczenie",
    wizardProgressLabel: "Postęp tłumaczenia",
    // Step 1 — upload
    uploadHeading: "Wybierz pliki KSeF do tłumaczenia",
    uploadHelpMulti: "Wgraj jedną lub wiele faktur FA(3) XML.",
    uploadCta: "Wybierz pliki",
    uploadDropHint: "Przeciągnij pliki lub wybierz z dysku",
    uploadFormatHint: "Obsługujemy XML FA(3) z KSeF (max 10 MB / plik)",
    addMoreFiles: "Dodaj pliki",
    filesReadyCountSingular: "1 plik gotowy do tłumaczenia",
    filesReadyCountPlural: "{count} plików gotowych do tłumaczenia",
    parsingRow: "Sprawdzam…",
    duplicateRow: "Już była tłumaczona — wybierz inną",
    duplicateNumberRow: "Możliwy duplikat: faktura {invoiceNumber} była już wgrana {count} razy — sprawdź, czy to nie pomyłka",
    removeFileLabel: "Usuń {filename}",
    continueCta: "Dalej",
    backCta: "Wstecz",
    clearAllCta: "Wyczyść",
    dataImmutableNotice: "Kwoty, NIP, IBAN i numery faktur NIE są tłumaczone przez AI.",
    // Step 2 — language & format
    languageHeading: "Wybierz język i format",
    languageHeadingForSingular: "Dla 1 faktury",
    languageHeadingForPlural: "Dla {count} faktur",
    targetLangLabel: "Język docelowy",
    targetLangSearchPlaceholder: "Szukaj języka…",
    showAllLanguages: "Pokaż wszystkie 22",
    formatLabel: "Format PDF",
    formatMono: "Tylko tłumaczenie",
    formatMonoHelp: "Faktura w jednym języku (np. angielskim).",
    formatBilingual: "Dwujęzycznie",
    formatBilingualHelp: "Wybrany język + polski — po jednej kolumnie obok siebie. Polecane dla kontrahenta + dokumentacji wewnętrznej.",
    costLabel: "Koszt",
    balanceLabel: "Stan",
    afterLabel: "Po",
    creditsUnit: "kredytów",
    creditsUnitSingular: "kredyt",
    cacheHitBadge: "Z cache — bez opłaty",
    polishSourceLocked: "Polski jest językiem źródłowym — wybierz inny język docelowy.",
    translateCtaSingular: "Tłumacz fakturę",
    translateCtaPlural: "Tłumacz {count} faktur",
    insufficientCreditsCta: "Doładuj kredyty",
    insufficientCreditsHint: "Po doładowaniu wrócisz tutaj automatycznie.",
    // Step 3 — delivery (single + batch)
    deliveryReadyTitle: "Tłumaczenie gotowe",
    deliveryReadySaved: "Zapisane",
    deliveryTranslatingTitle: "Tłumaczę fakturę",
    deliveryTranslatingHint: "Etykiety pochodzą ze słowników, AI tłumaczy tylko wolny tekst (opisy, notatki, stopkę).",
    deliveryElapsedSeconds: "{seconds} s",
    deliveryCancelCta: "Anuluj tłumaczenie",
    deliveryRowErrorTitle: "Tłumaczenie nie powiodło się",
    deliveryRowErrorHint: "Możesz spróbować ponownie — kredyt został zwrócony.",
    deliveryBatchTitleSingular: "Tłumaczę 1 fakturę na {language}",
    deliveryBatchTitlePlural: "Tłumaczę {count} faktur na {language}",
    deliveryBatchBilingual: "(dwujęzycznie)",
    progressCount: "{done} / {total} ukończonych",
    progressEta: "~{seconds} s",
    queuedLabel: "w kolejce",
    translatingLabel: "tłumaczę…",
    cancelledLabel: "Anulowano przez użytkownika",
    cancelBatchCta: "Zatrzymaj",
    resumeBatchCta: "Wznów",
    retryCta: "Ponów",
    previewCta: "Podgląd",
    downloadCta: "Pobierz",
    downloadPdfCta: "Pobierz PDF",
    downloadAllZipCta: "Pobierz wszystkie (.zip)",
    translateAgainCta: "Tłumacz ten zestaw w innym języku",
    changeLanguageCta: "Zmień język",
    newTranslationCta: "Nowe tłumaczenie",
    editTranslationCta: "Edytuj",
    editorTitle: "Edytuj tłumaczenie",
    editorSubtitle: "Popraw wolny tekst — kwoty i numery nie są edytowane.",
    editorItemNameLabel: "Nazwa pozycji",
    editorItemUnitLabel: "Jednostka",
    editorNotesLabel: "Notatki",
    editorFooterLabel: "Stopka",
    editorAdditionalLabel: "Dodatkowe opisy",
    editorAdditionalKeyLabel: "Etykieta",
    editorAdditionalValueLabel: "Treść",
    editorCorrectionLabel: "Korekta",
    editorCorrectionReasonLabel: "Powód korekty",
    editorCorrectionPeriodLabel: "Okres korekty",
    editorOrdersLabel: "Pozycje zamówienia",
    editorOrderLabel: "Zamówienie",
    editorOrderLineNameLabel: "Nazwa pozycji zamówienia",
    editorOrderLineUnitLabel: "Jednostka zamówienia",
    editorSettlementsLabel: "Rozliczenia",
    editorSettlementChargeLabel: "Powód obciążenia",
    editorSettlementDeductionLabel: "Powód potrącenia",
    editorFragmentsLabel: "Dodatkowe fragmenty dokumentu",
    editorSaveCta: "Zapisz zmiany",
    editorSavingCta: "Zapisuję…",
    editorCancelCta: "Anuluj",
    editorCloseCta: "Zamknij",
    editorNoFieldsHint: "Brak edytowalnych pól w tym tłumaczeniu.",
    editorSavedToast: "Zapisano zmiany.",
    editorSaveFailedToast: "Nie udało się zapisać.",
    editorResetCta: "Przywróć automatyczne",
    editorResetConfirm: "Cofnąć zmiany do wersji AI?",
    translateRowError: "Tłumaczenie nie powiodło się"
  },
  en: {
    navProduct: "Product",
    navHow: "How it works",
    navPricing: "Pricing",
    navFaq: "FAQ",
    uiLanguage: "Interface language",
    badge: "Utility for Polish companies working with KSeF",
    headline: "Translate KSeF invoices into professional invoices for international contractors.",
    subheadline:
      "Upload FA(3) XML from KSeF, review a readable invoice, translate free text, and export a polished PDF without KSeF login, ERP, or accounting workflows.",
    ctaUpload: "Upload invoice",
    ctaHow: "See workflow",
    trust1: "Does not issue invoices",
    trust2: "Does not connect to KSeF",
    trust3: "Does not alter financial data",
    uploadTitle: "Upload KSeF FA(3) XML",
    uploadHelp: "Drag and drop, or choose a file from your computer.",
    parserFeature: "FA(3) KSeF XML parser",
    translationFeature: "Label and free-text translation",
    pdfFeature: "Unicode PDF export with QR",
    targetLanguage: "Invoice target language",
    bilingual: "Bilingual: selected language + Polish",
    translate: "Translate descriptions",
    download: "Download PDF",
    parsing: "Parsing KSeF invoice...",
    empty: "Upload a FA(3) XML invoice from KSeF to generate the preview in",
    noAi:
      "Static labels are translated from dictionaries. Add OPENAI_API_KEY to translate item descriptions, units, notes, and footer text.",
    parsePdfFailed: "We could not read the file.",
    translationFailed: "Translation failed.",
    pdfFailed: "PDF generation failed.",
    workspaceTitle: "Invoice converter",
    workspaceLead:
      "This is the application workspace. Upload a file, select the contractor language, choose mono- or bilingual output, and export the PDF.",
    howTitle: "How it works",
    howLead: "A focused workflow with no deployment or integration required.",
    steps: [
      ["Upload file", "FA(3) XML files downloaded from KSeF are supported."],
      ["Review preview", "The app normalizes invoice data into one internal model and renders it clearly."],
      ["Translate text", "Fixed labels use dictionaries; AI translates only descriptions, notes, and free text."],
      ["Export PDF", "The export includes parties, line items, payment, footer, KSeF QR, and translation disclaimer."]
    ],
    valueTitle: "Built for contractor communication",
    valueLead:
      "This is not accounting or ERP software. It is a focused utility for readable invoice representations for foreign recipients.",
    values: [
      ["Safe scope", "We do not issue, book, or modify the source invoice."],
      ["Value control", "Amounts, dates, invoice numbers, VAT IDs, IBANs, and VAT rates are never translated by AI."],
      ["European languages", "Major European languages are supported and more can be added centrally."],
      ["KSeF-ready", "The mapper handles FA(3), advances, payments, bank accounts, footers, and verification QR links."]
    ],
    pricingTitle: "MVP pricing",
    pricingLead: "A simple model for companies sending readable invoices to foreign partners.",
    plans: [
      ["Starter", "€0", "Testing and validation", ["XML import", "Invoice preview", "Sample export", "No user accounts"]],
      ["Business", "€35 / mo", "For sales and administration teams", ["Unlimited translations", "Branded PDFs", "Local or company history", "Priority EU languages"]],
      ["Enterprise", "Custom", "For larger invoice volumes", ["SLA and onboarding", "Security policies", "Internal integrations", "Custom PDF templates"]]
    ],
    faqTitle: "FAQ",
    faqs: [
      ["Does it connect to KSeF?", "No. Users upload XML files. The app only parses, translates, renders, and exports."],
      ["Does it replace accounting?", "No. It creates a readable representation for contractors, not bookkeeping records."],
      ["What does AI translate?", "Only free text: line item descriptions, service names, notes, footer, and payment instructions."],
      ["Can amounts or identifiers change?", "They should not. Financial values, IDs, dates, currencies, IBANs, and VAT rates are copied, not translated by AI."]
    ],
    seoTitle: "KSeF invoice translator, FA(3) viewer, XML invoice to PDF",
    seoBody:
      "KSeF Invoice Translator helps Polish companies prepare professional KSeF invoice translations for contractors. It supports FA(3) XML invoice files, Polish invoice translation, translate KSeF invoice workflows, and invoice PDF export for international partners.",
    balanceFree: "Free credit",
    balanceFreePaid: "credits",
    outOfCreditsTitle: "Out of credits",
    outOfCreditsBody:
      "You have used your free invoice for this month and have no credit pack. Buy a pack to continue — the free credit refreshes on the 1st of each month.",
    buyCredits: "Buy credits",
    cancel: "Cancel",
    close: "Close",
    drawerTitle: "Top up credits",
    billingPlaceholderTitle: "Credit packs",
    billingPlaceholderBody: "The credit-pack store will be available in the next phase.",
    billingTitle: "Credit packs",
    billingSubtitle: "The free credit refreshes on the 1st of each month. Pack credits never expire.",
    billingBandPaidLabel: "credits",
    billingBandFreeLabel: "free this month",
    billingBandNextFreeLabel: "Next free credit refreshes:",
    billingIncludedHeading: "What's included?",
    billingVatNote: "All prices net of VAT. 23% VAT added at checkout.",
    billingRefundPolicy: "Unused credits never expire. Refunds on request within 14 days.",
    pickPackage: "Pick a pack size",
    unitPrice: "per invoice (net)",
    total: "Total (net)",
    totalWithTax: "With 23% VAT",
    continueToCheckout: "Continue to checkout",
    purchaseHistory: "Purchase history",
    purchaseDate: "Date",
    purchaseSize: "Pack",
    purchaseTotal: "Amount",
    purchaseStatus: "Status",
    purchaseStatusPaid: "Paid",
    purchaseStatusPending: "Pending",
    purchaseStatusFailed: "Failed",
    purchaseStatusRefunded: "Refunded",
    fakturaHeader: "Invoice",
    fakturaInProgress: "In progress",
    fakturaIssued: "Issued ✓",
    fakturaError: "Error",
    fakturaDownload: "Download PDF →",
    paymentSuccessTitle: "Payment complete",
    paymentSuccessBody: "Credits have been added to your account.",
    paymentCancelledTitle: "Payment cancelled",
    paymentCancelledBody: "The payment was cancelled. You can try again.",
    topUp: "Top up",
    creditsExhaustedShort: "Out of credits",
    lowBalanceBannerTitle: "Out of credits",
    lowBalanceBannerBody:
      "You have used your free invoice this month and have no credit pack. Buy a pack to upload another invoice.",
    newInvoice: "New invoice",
    recentHeading: "Recent",
    allArchive: "Full archive",
    helpLabel: "Help",
    contactLabel: "Contact",
    tryWithSample: "Try with sample invoice",
    onboardingTitle: "What you get",
    onboardingItem1: "1 free invoice per month",
    onboardingItem2: "Translation to 20+ languages",
    onboardingItem3: "Bilingual PDF with KSeF QR",
    onboardingItem4: "No KSeF login, no integration required",
    cached: "cached",
    moreLanguages: "More languages",
    originalPolish: "Polish",
    navWorkspace: "Workspace",
    navHistory: "History",
    signOut: "Log out",
    accountTitle: "Account",
    accountProfileHeading: "Profile",
    accountEmailLabel: "Email",
    accountEmailHelp: "Sign-in email cannot be changed.",
    accountLocaleLabel: "Interface language",
    accountDisplayNameLabel: "Display name (optional)",
    accountDisplayNameHelp: "Used in transactional emails.",
    accountFirstNameLabel: "First name",
    accountLastNameLabel: "Last name",
    accountFullNameHelp: "Your first and last name appear on every translated invoice as the verifier — a Ministry of Finance requirement.",
    nameCaptureTitle: "Verifier name for translated invoices",
    nameCaptureBody:
      "Translated documents must show the name of the person who carried out or verified the translation — a Ministry of Finance condition for accepting a translated document. Once you fill these in we'll replace the email on each invoice with your name.",
    nameCaptureFirstNameLabel: "First name",
    nameCaptureLastNameLabel: "Last name",
    nameCaptureSave: "Save and continue",
    nameCaptureSaving: "Saving…",
    nameCaptureDismiss: "Later",
    nameCaptureError: "Could not save. Please try again.",
    accountSaveButton: "Save changes",
    accountSavingButton: "Saving…",
    accountSaveSuccess: "Saved.",
    accountSaveError: "Could not save changes.",
    accountExportHeading: "Data export (GDPR)",
    accountExportBody: "Download a full export of your data in JSON format.",
    accountExportButton: "Download data (JSON)",
    accountExportPreparing: "Preparing…",
    accountDangerHeading: "Danger zone",
    accountDeleteAccountTitle: "Delete account",
    accountDeleteAccountBody: "All your data (invoices, translations, purchase history) will be permanently deleted.",
    accountDeleteAccountButton: "Delete account",
    accountDeleteConfirmTitle: "Confirm account deletion",
    accountDeleteConfirmBody: "Type your email address to confirm. This action cannot be undone.",
    accountDeleteConfirmPlaceholder: "Type your email address",
    accountDeleteConfirmAction: "Yes, permanently delete my account",
    accountDeleteCancel: "Cancel",
    // -------------------------------------------------------------------
    // Tłumacz wizard (spec 2026-05-20). Mirror of the pl block above —
    // every key here must exist there too (enforced by the type below).
    // -------------------------------------------------------------------
    wizardStepUpload: "Choose files",
    wizardStepLanguage: "Language & format",
    wizardStepDelivery: "Translation",
    wizardProgressLabel: "Translation progress",
    // Step 1 — upload
    uploadHeading: "Choose KSeF invoices to translate",
    uploadHelpMulti: "Upload one or many FA(3) XML invoices.",
    uploadCta: "Choose files",
    uploadDropHint: "Drop files here or pick from disk",
    uploadFormatHint: "We accept FA(3) XML from KSeF (max 10 MB / file)",
    addMoreFiles: "Add files",
    filesReadyCountSingular: "1 file ready to translate",
    filesReadyCountPlural: "{count} files ready to translate",
    parsingRow: "Parsing…",
    duplicateRow: "Already translated — pick another",
    duplicateNumberRow: "Possible duplicate: invoice {invoiceNumber} has been uploaded {count} time(s) before — double-check",
    removeFileLabel: "Remove {filename}",
    continueCta: "Continue",
    backCta: "Back",
    clearAllCta: "Clear",
    dataImmutableNotice: "Amounts, tax IDs, IBANs and invoice numbers are NOT translated by AI.",
    // Step 2 — language & format
    languageHeading: "Choose language and format",
    languageHeadingForSingular: "For 1 invoice",
    languageHeadingForPlural: "For {count} invoices",
    targetLangLabel: "Target language",
    targetLangSearchPlaceholder: "Search language…",
    showAllLanguages: "Show all 22",
    formatLabel: "PDF format",
    formatMono: "Translation only",
    formatMonoHelp: "Invoice in a single language (e.g. English).",
    formatBilingual: "Bilingual",
    formatBilingualHelp: "Chosen language + Polish — side by side. Recommended for contractor + internal records.",
    costLabel: "Cost",
    balanceLabel: "Balance",
    afterLabel: "After",
    creditsUnit: "credits",
    creditsUnitSingular: "credit",
    cacheHitBadge: "From cache — free",
    polishSourceLocked: "Polish is the source language — choose a different target.",
    translateCtaSingular: "Translate invoice",
    translateCtaPlural: "Translate {count} invoices",
    insufficientCreditsCta: "Top up credits",
    insufficientCreditsHint: "You'll come back here automatically after topping up.",
    // Step 3 — delivery (single + batch)
    deliveryReadyTitle: "Translation ready",
    deliveryReadySaved: "Saved",
    deliveryTranslatingTitle: "Translating invoice",
    deliveryTranslatingHint: "Labels come from dictionaries; AI translates only free text (descriptions, notes, footer).",
    deliveryElapsedSeconds: "{seconds}s",
    deliveryCancelCta: "Cancel translation",
    deliveryRowErrorTitle: "Translation failed",
    deliveryRowErrorHint: "You can retry — the credit was refunded.",
    deliveryBatchTitleSingular: "Translating 1 invoice to {language}",
    deliveryBatchTitlePlural: "Translating {count} invoices to {language}",
    deliveryBatchBilingual: "(bilingual)",
    progressCount: "{done} / {total} done",
    progressEta: "~{seconds}s",
    queuedLabel: "queued",
    translatingLabel: "translating…",
    cancelledLabel: "Cancelled by user",
    cancelBatchCta: "Stop",
    resumeBatchCta: "Resume",
    retryCta: "Retry",
    previewCta: "Preview",
    downloadCta: "Download",
    downloadPdfCta: "Download PDF",
    downloadAllZipCta: "Download all (.zip)",
    translateAgainCta: "Translate this set in another language",
    changeLanguageCta: "Change language",
    newTranslationCta: "New translation",
    editTranslationCta: "Edit",
    editorTitle: "Edit translation",
    editorSubtitle: "Refine free-text fields — amounts and numbers are not editable.",
    editorItemNameLabel: "Item name",
    editorItemUnitLabel: "Unit",
    editorNotesLabel: "Notes",
    editorFooterLabel: "Footer",
    editorAdditionalLabel: "Additional descriptions",
    editorAdditionalKeyLabel: "Label",
    editorAdditionalValueLabel: "Content",
    editorCorrectionLabel: "Correction",
    editorCorrectionReasonLabel: "Correction reason",
    editorCorrectionPeriodLabel: "Correction period",
    editorOrdersLabel: "Order lines",
    editorOrderLabel: "Order",
    editorOrderLineNameLabel: "Order line name",
    editorOrderLineUnitLabel: "Order line unit",
    editorSettlementsLabel: "Settlements",
    editorSettlementChargeLabel: "Charge reason",
    editorSettlementDeductionLabel: "Deduction reason",
    editorFragmentsLabel: "Additional document fragments",
    editorSaveCta: "Save changes",
    editorSavingCta: "Saving…",
    editorCancelCta: "Cancel",
    editorCloseCta: "Close",
    editorNoFieldsHint: "No editable fields in this translation.",
    editorSavedToast: "Saved.",
    editorSaveFailedToast: "Save failed.",
    editorResetCta: "Restore AI version",
    editorResetConfirm: "Revert edits to the AI version?",
    translateRowError: "Translation failed"
  }
} satisfies Record<UiLanguage, Record<string, unknown>>;

export type Copy = (typeof copy)[UiLanguage];
